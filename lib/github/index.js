...
github.checkGithubToken = async (cache) => {
  if (cache.githubAuth.token) {
    ...
    try {
      ...
      helpers.addTo(cache, {githubAuth})
      spinner.succeed(`Github token is correct. ${chalk.dim('(It will be used for Github release)')}`)
      return cache
    } catch (err) {
...
github.checkGithubCredentials = async (cache) => {
  spinner.text = `Checking Github credentials`
  spinner.start()
  try {
    octokit.authenticate({
      type: 'basic',
    ...
    await octokit.users.get({})
    githubAuth.credentials = true
    helpers.addTo(cache, {githubAuth})
    spinner.succeed()
    return cache
  } catch (err) {
    spinner.fail('Wrong Github credentials')
    console.log(chalk.red('Try again with correct Github username and password'))
    process.exit(1)
  }
}
