        ...
        if (helpers.isChecked(cache, ['npm', 'github+Npm'])) {
            if (cache.pkgPath) {
                ...
                fs.writeFile(cache.pkgPath, pkgStr, function(err) {
                    if (err) {throw err;}
                    // console.log(`${ chalk.yellow(file) } version successfully bumped from ${ chalk.green(oldVersion) } to ${ chalk.yellow(cache.newVersion) }`);
                    resolve(cache);
                });
            } else {
                resolve(cache);
            ...
            if (cache.isNpmPublish && cache.npmLogin) {
                ...
                shell.exec(str, { async:true, silent:true }, function(err) {
                    if (err) {
                        ...
                        console.log(chalk.green('NPM package successfully published'));
                    }
                    spinner.stop();
                    process.stdout.write('\n');
                    resolve(cache);
