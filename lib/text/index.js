...
{red Need to install new version of Node.js} {green >= ${variables.env.node}}
`)
...
`)
text.gitAdd = (cache) => (chalk`{yellow ${cache.notStagedFilesPerLine}} => not staged for commit. Add those files as a part of a release commit? {grey.dim (git add .)}`)
