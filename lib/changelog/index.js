...
changelog.getCommitMessages = function getCommitMessages(cache) {
    return new Promise(function(resolve) {
        ...
        _(commitMessages.commitMessages).forEach(function(value) {
            var val = value;
            _(breakingChangesLabels).forEach(function(value) {
                if (_.startsWith(val, value)) {
                    ...
                    var matchBreakingChangesPrefix = val.match(/^([^\]]+)([^])/);
                    ...
                    var prefix = message.match(/^([^\]]+)([^])/)[0];
                    message = _.replace(message, prefix, '').trim();
                    changelog.breakingChanges.push({ hash: hash, prefix: prefix, message: message, breakingChangesPrefix: breakingChangesPrefix });
                }
            });
        });
        if (_.isEmpty(changelog.breakingChanges)) {
            changelog.breakingChanges = false;
        }
        ...
        if (changelog.breakingChanges) {
            _(cache.breakingChanges).forEach(function(value) {
                breakingChangesMessage = breakingChangesMessage + '- **' + value.breakingChangesPrefix + '** ' + '**' + value.prefix + '** ' + value.message + ' ([' + value.hash + '](' + cache.remote + '/commit/' + value.hash + '))' + '\n';
            });
            breakingChangesMessage = ':heavy_exclamation_mark:**Breaking Changes:**' + '\n' + breakingChangesMessage + '\n';
        }
