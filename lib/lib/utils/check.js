...
async function checkRemoteRepo (data) {
  if (data.git.isGitRemote) {
    if (data.git.isGitBranchBehind) {
      console.error(`${chalk.bold.red('Error:')} Your branch is behind ${chalk.cyan(data.git.gitBranchUpstream)}\n`)
      process.exit(1)
    }
    if (data.git.isGitBranchAhead) {
      console.error(`${chalk.bold.red('Error:')} Your branch is ahead ${chalk.cyan(data.git.gitBranchUpstream)}\n`)
      process.exit(1)
    }
    ...
    if (data.git.gitRemoteOrigin.protocol && data.git.gitRemoteOrigin.protocol !== 'ssh') {
      ...
      console.error(`${chalk.red('Warning:')} You are using non-SSH remote ${chalk.cyan(data.git.gitRemoteOrigin.href)}`)
...
//   })
//   if (!permissions.push) {
//     console.error(`${chalk.bold.red('Error:')} GITHUB_TOKEN not valid\n`)
//     process.exit(1)
//   }
// }
