...
git.notStagedFiles = async (cache) => {
  ...
  if (_.isEmpty(data.stdout)) {
    ...
    notStagedFilesPerLine = {notStagedFilesPerLine: notStagedFiles.notStagedFiles.map((notStagedFile, i) => i === 0 ? notStagedFile : `\n  ` + notStagedFile)}
  }
  helpers.addTo(cache, notStagedFiles)
  helpers.addTo(cache, notStagedFilesPerLine)
  return cache
}
